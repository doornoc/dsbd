...
from dsbd.ticket.models import Ticket, Chat, Template
...
def template_model_pre_save(sender, instance, **kwargs):
    try:
        instance._pre_save_instance = Template.objects.get(pk=instance.pk)
    except Template.DoesNotExist:
        instance._pre_save_instance = instance


@receiver(post_save, sender=Template)
def post_template(sender, instance, created, **kwargs):
    if created:
        text = get_create_template(True, instance)
        notify_db_save(table_name="Template", type=0, data=text)
    else:
        text = get_update_template(instance._pre_save_instance, instance)
        notify_db_save(table_name="Template", type=1, data=text)


@receiver(pre_delete, sender=Template)
def delete_template(sender, instance, **kwargs):
    text = get_create_template(False, instance)
    notify_db_save(table_name="Template", type=2, data=text)


def get_create_template(short, instance):
    return '--%d[%s]--\n' % (instance.id, instance.title)


def get_update_template(before, after):
    text = '%s----更新状況----\n' % (get_create_template(True, before),)
    if before.number != after.number:
        text += '順序: %d => %d\n' % (before.number, after.number)
    if before.is_active != after.is_active:
        text += '有効: %s => %s\n' % (active_to_str(before.is_active), active_to_str(after.is_active))
    if before.type1 != after.type1:
        text += 'type1: %s => %s\n' % (before.type1, after.type1)
    if before.type2 != after.type2:
        text += 'type2: %s => %s\n' % (before.type2, after.type2)
    if before.title != after.title:
        text += 'title: %s => %s\n' % (before.title, after.title)
    if before.body != after.body:
        text += 'body: %s => %s\n' % (before.body, after.body)
